...
const work = mkdtempSync(join(tmpdir(), "drift-bench-repo-"));
...
const marker = (i) => (i % 7 === 0 ? `\n\nDrift-Intent: ${id(i)}` : ""); // 1/7 of commits carry a trailer
const malformedIdx = new Set(
  Array.from({ length: Math.floor(COMMITS * malformedRate) }, (_, k) => Math.floor((k * 977) % COMMITS)),
);

console.error(`generating ${COMMITS} commits (malformed rate ${malformedRate}) via fast-import...`);
const FILE_CONTENT = "export const app = 1;\n";
const streamFile = join(work, "fi-stream.txt");
const fs = await import("node:fs");
let stream = "";
const FLUSH = 5_000; // append to the file every N commits (bounded memory)
let mark = 1;
for (let i = 1; i <= COMMITS; i++) {
  const malformed = malformedIdx.has(i);
  const msg = `chore(bench): commit ${i}${marker(i)}`;
  ...
  const mf = manifestJson(i, malformed);
  stream += `M 100644 inline ${INTENTS_DIR}/${id(i)}.json\ndata ${Buffer.byteLength(mf, "utf8")}\n${mf}\n`;
...
const imp = spawnSync("git", ["fast-import", "--quiet"], {
  cwd: repo,
...
const output = {
  scenario: { commits: COMMITS, manifests: COMMITS, malformedRate, generator: "git fast-import", machine: `${process.platform} ${process.arch}` },
...
if (asJson) {
  ...
  console.log(`\n# Large-repo benchmark — ${COMMITS.toLocaleString()} commits / manifests\n`);

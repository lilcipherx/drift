...
## Follow-up (same day, 2026-08-16)

All items from §7 were executed in this session:

- **C1 (prompt in commit message) — fixed:** `[prompts] mode` with
  `commit-summary` as the safe default; `full` / `none` opt-ins. Tests +
  docs + SECURITY.md updated. Tests now 123/123.
- **C2 (no CI) — fixed:** `.github/workflows/ci.yml` (ubuntu + windows × Node
  24, npm ci → build → test → eval → acceptance → pack smoke). ADR-008
  amended.
- **C3 (Action posts nothing) — fixed:** `comment` input posts an idempotent
  provenance summary via `scripts/pr-comment.mjs` (5 unit tests).
- **§3 items:** `drift status` added; human `blame` output reworked
  (Why / Generated by / …); npm metadata + `docs/NPM_RELEASE.md`;
  bug/feature issue templates + `docs/GOOD_FIRST_ISSUES.md`;
  CONTRIBUTING.md rewritten; README redesigned + `docs/installation.md`;
  stale test counts fixed (README badge, PR template, NEXT_STEPS).
- **Remaining external blocker:** npm publication (owner credentials) and
  enabling GitHub Actions/Issues on the account (see `docs/NPM_RELEASE.md`

...
- **Horizontal scale** requires the shared-queue production adapter
  (`QueueAdapter` interface; a Redis/Postgres-backed adapter is an
  implementation task — see §5 "Migrating to a shared queue"). Until then the
  capacity envelope (CAPACITY_MODEL) is single-node.
...
| Inspect queue | queue DB: `SELECT status, COUNT(*) FROM webhook_jobs GROUP BY status;` |
...
| Scale workers | raise `DRIFT_APP_CONCURRENCY`; add replicas (shared queue required for >1 replica) |
...
## 5. Migrating to a shared queue (production horizontal scale)

1. Implement a `QueueAdapter` backed by the chosen durable store (Redis
   streams / Postgres `FOR UPDATE SKIP LOCKED`) with the same semantics:
   delivery-id unique enqueue, claim/lease/visibility timeout, ack/nack with
   backoff, dead-letter status, `depth()`, `stats()`, crash re-claim.
2. Contract tests: `tests/app/queue.test.mjs` runs against `QueueAdapter` —
   the new adapter must pass the same suite.
3. Rollback: the webhook server is adapter-agnostic; switching back to the
...
| Key import | `drift key import --file <path>` (exact match enforced) |

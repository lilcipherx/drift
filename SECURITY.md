...
Neutral states — valid bootstrap, unsigned/unverifiable/missing legacy
manifests, no intents — never fail by default. An initial trust-root
bootstrap (base has no key, head introduces the first key) is VISIBLE in the
step summary/comment and stays neutral; replacement/removal are blocking
failures.

...
### Added
- **`drift status`** — first-run-friendly repo state (intents, head, prompt
  mode, encryption, git branch) with a "Next:" hint; degrades gracefully
  before `init` (reason `no-git` / `not-initialized`, exit 1).
- **Prompt storage modes** (`[prompts] mode` in `.drift/config.toml`):
  `commit-summary` (default — full prompt only in the local, gitignored
  `.drift/` store; the git commit message carries only `Intent:` first line
  ≤72 chars + `Model:` / `Verification:` / `Drift-Intent:` trailers),
  `full` (prompt also in the commit message — legacy behaviour), `none`
  (prompt stored nowhere; generic `Intent recorded` subject). The summary is
  derived from the already-redacted prompt, so secrets cannot leak through
  it. Encryption at rest applies on top of any mode. New intents only —
...
- `drift blame` human output is now self-explanatory: labelled sections
  (`Why:` / `Generated by:` / `Verification:` / `Intent:` / `Commit:`) with
  the recorded `--verify-cmd` shown when present. JSON output unchanged.

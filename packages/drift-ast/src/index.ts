...
/**
 * The optional TypeScript compiler. Two generations are supported:
 *   - TypeScript ≤ 5.x — ships the full JS API (`transpileModule`, …) which
 *     `drift realize` uses for real parse-only validation.
 *   - TypeScript 7.x — the native (Go) compiler: the JS API is REMOVED and
 *     only the `tsc` CLI exists. Validation falls back to a `tsc --noCheck`
 *     subprocess on a temp file, which reports parse errors only (type
 *     errors never block commits, matching the transpile-only contract).
 * The types are deliberately loose so this package compiles against both.
 */
interface TsApi {
  transpileModule(
    source: string,
    options: {
      ...
      compilerOptions: { allowJs: boolean; target: number; jsx: number };
    },
  ): { diagnostics?: { category: number; start?: number; messageText: string }[] };
  ScriptTarget: Record<string, number>;
  JsxEmit: Record<string, number>;
  DiagnosticCategory: { Error: number };
  flattenDiagnosticMessageText(messageText: string, newline: string): string;
}

function loadTypeScript(): TsApi | null {
  try {
    const ts = require_("typescript") as unknown as Partial<TsApi>;
...
function tscBinPath(): string | null {
  try {
    ...
    const pkg = require_("typescript/package.json") as { bin?: { tsc?: string } | string };
 ...
 */
function validateSyntaxWithTscCli(source: string): string | null {
...
export function validateSyntax(
  ...
  if (language === "typescript") {
    ...
    if (!ts) return validateSyntaxWithTscCli(source); // TS 7 native compiler

...
export interface TrustSet {
  ...
  active: { fingerprint: string; pem: string }[];
...
export function keyringPayload(
  seq: number,
  action: KeyringAction,
  fingerprint: string,
  by: string,
...
export function parseKeyringKey(
  pem: string,
): { ok: true; fingerprint: string; pem: string } | { ok: false; error: string } {
  if (!pem || pem.trim().length === 0) return { ok: false, error: "empty key PEM" };
...
export function keyringFingerprint(pem: string): string {
  return signingKeyIdFor(pem);
}

function isKeyringAction(v: unknown): v is KeyringAction {
 ...
 */
export function validateKeyring(
  raw: string | null | undefined,
  anchorPem: string | null,
): { ok: true; keyring: KeyringFile } | { ok: false; error: string } {
  ...
  if (typeof json !== "object" || json === null) return { ok: false, error: "keyring.json is not an object" };
  ...
  for (const k of file.keys as unknown[]) {
    if (typeof k !== "object" || k === null) return { ok: false, error: "keyring.json contains a non-object key entry" };
    ...
    if (!parsed.ok) return { ok: false, error: `key entry ${e.fingerprint}: ${parsed.error}` };
    ...
    if (seen.has(e.fingerprint)) return { ok: false, error: `duplicate key fingerprint ${e.fingerprint}` };
  ...
  if (entries.length === 0) return { ok: false, error: "keyring.json has no keys" };

  // --- audit replay -------------------------------------------------------
  // Replay state: fingerprint → entry. bootstrap/add insert, revoke marks,
  // remove deletes. The final map must exactly match `entries`.
  const replay = new Map<string, KeyringEntry>();
  let seq = 0;
  for (const a of file.audit as unknown[]) {
    if (typeof a !== "object" || a === null) return { ok: false, error: "audit contains a non-object entry" };
    ...
    const action = e.action as KeyringAction;
    if (action === "bootstrap") {
      if (seq !== 1) return { ok: false, error: "bootstrap entry must be seq 1" };
      if (!anchorPem) return { ok: false, error: "keyring exists but no anchor key (.drift/public/key.pem)" };
      const anchor = parseKeyringKey(anchorPem);
      if (!anchor.ok) return { ok: false, error: `anchor key invalid: ${anchor.error}` };
      ...
      if (replay.has(e.fingerprint)) return { ok: false, error: `bootstrap adds an already-present key ${e.fingerprint}` };
    ...
    if (action === "add") {
      if (target) return { ok: false, error: `add for already-known fingerprint ${e.fingerprint}` };
      ...
      if (!entry) return { ok: false, error: `added fingerprint ${e.fingerprint} has no key entry` };
      if (entry.addedBy !== e.by) {
        return { ok: false, error: `added key ${e.fingerprint} addedBy does not match the audit authorizer` };
      }
      if (e.by === e.fingerprint) return { ok: false, error: "a key cannot add itself" };
  ...
  if (seq === 0) return { ok: false, error: "keyring.json has an empty audit log" };
  ...
  return {
    ok: true,
    keyring: { schemaVersion: 1, keys: entries, audit: file.audit as KeyringAuditEntry[] },
  };
}
...
export type KeyringChangeResult =
  | { ok: true; keyring: KeyringFile; entry: KeyringAuditEntry }
 ...
 */
export function applyKeyringChange(
  keyring: KeyringFile,
  privateKeyPem: string,
  action: KeyringChangeAction,
  target: { pem?: string; fingerprint?: string },
  reason: string | null,
  at: number = Date.now(),
): KeyringChangeResult {
  ...
  if (!authorizer.ok) return { ok: false, error: "the private key does not derive a valid Ed25519 public key" };
  ...
  if (action === "add") {
    if (!target.pem) return { ok: false, error: "add requires the new key PEM" };
    const parsed = parseKeyringKey(target.pem);
    if (!parsed.ok) return { ok: false, error: `new key invalid: ${parsed.error}` };
    ...
    if (fingerprint === by) return { ok: false, error: "a key cannot add itself to the keyring" };
  } else {
    if (!target.fingerprint) return { ok: false, error: `${action} requires a fingerprint` };
    fingerprint = target.fingerprint;
    const known = keyring.keys.find((k) => k.fingerprint === fingerprint);
    if (!known) return { ok: false, error: `fingerprint ${fingerprint} is not in the keyring` };
    if (action === "revoke") {
  ...
  const keys: KeyringEntry[] = keyring.keys.map((k) => ({ ...k }));
  if (action === "add") {
    keys.push({
      fingerprint,
      pem: (target.pem as string).trim(),
      status: "active",
      addedBy: by,
      addedAt: at,
      transitionedBy: null,
...
export function keyringPath(driftDir: string): string {
 ...
 */
export function loadTrustSet(driftDir: string): TrustSet {
  const anchorPath = join(driftDir, "public", "key.pem");
  const krPath = keyringPath(driftDir);
  let anchorPem: string | null = null;
  ...
  if (!keyringPresent) {
    ...
    if (!anchorPem) return { anchorPem: null, active: [], keyring: null, malformed: null, keyringPresent: false };
...
export function writeKeyringFile(driftDir: string, keyring: KeyringFile): void {
...
export function findKeyringEntry(keyring: KeyringFile, fingerprint: string): KeyringEntry | undefined {
...
function auditEntriesEqual(a: KeyringAuditEntry, b: KeyringAuditEntry): boolean {
  return (
    a.seq === b.seq &&
    a.action === b.action &&
...
function isStrictPrefix(base: KeyringFile, head: KeyringFile): boolean {
  if (base.audit.length >= head.audit.length) return false;
  for (let i = 0; i < base.audit.length; i++) {
    if (!auditEntriesEqual(base.audit[i]!, head.audit[i]!)) return false;
  }
  return true;
}

/** True when two keyrings are byte-identical (after strict parse). */
function keyringsEqual(a: KeyringFile, b: KeyringFile): boolean {
  if (a.audit.length !== b.audit.length) return false;
  for (let i = 0; i < a.audit.length; i++) {
    if (!auditEntriesEqual(a.audit[i]!, b.audit[i]!)) return false;
  }
  return true;
}
 ...
 */
export function evaluateKeyringChange(
  baseRaw: string | null | undefined,
  headRaw: string | null | undefined,
  baseAnchor: string | null | undefined,
  headAnchor: string | null | undefined,
): KeyringChange {
  ...
  if (!basePresent && headPresent) {
    ...
    if (!head.ok) return "malformed-bootstrap";
    return "bootstrap";
  }
  const base = validateKeyring(baseText, baseAnchorPresent ? baseAnchorText : null);
  if (!base.ok) return "base-malformed";
  if (!headPresent) return "removed";
  const head = validateKeyring(headText, headAnchorPresent ? headAnchorText : null);
  if (!head.ok) return "malformed-replacement";
  if (keyringsEqual(base.keyring, head.keyring)) return "unchanged";
  // The only legitimate change is a strict append-only extension.
  if (isStrictPrefix(base.keyring, head.keyring)) return "extended";
  return "replaced";
}

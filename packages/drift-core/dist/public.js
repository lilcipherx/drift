...
export class PublicStore {
    ...
    topNewest(limit, filters = {}) {
        ...
        const insert = (v) => {
            ...
            while (lo < hi) {
                const mid = (lo + hi) >>> 1;
                if (kept[mid].timestamp < v.timestamp)
                    hi = mid; // strictly older goes after
                else
                    lo = mid + 1;
            }
            kept.splice(lo, 0, v);
            if (kept.length > L)
                kept.length = L; // drop the oldest (tail)
        };
        for (const name of readdirSync(dir)) {
            ...
            if (filters.author !== undefined && (v.agent?.identifier ?? "unknown") !== filters.author)
                continue;
            if (filters.model !== undefined && (v.model ?? null) !== filters.model)
                continue;
            const files = (v.files ?? []).map((f) => f.path);
            if (filters.filePrefix !== undefined && !files.some((p) => p.startsWith(filters.filePrefix)))
                continue;
            if (filters.fileExact !== undefined && !files.some((p) => p === filters.fileExact))
                continue;
            insert(v);
        }
        return kept;

 ...
 */
import { createCipheriv, createDecipheriv, createHash, createPrivateKey, createPublicKey, generateKeyPairSync, randomBytes, sign as nodeSign, verify as nodeVerify, } from "node:crypto";
...
export function verifyPayload(payload, publicKeyPem, signatureBase64) {
    try {
        const key = createPublicKey(publicKeyPem);
        return nodeVerify(null, Buffer.from(payload, "utf8"), key, Buffer.from(signatureBase64, "base64"));
    }
    catch {
        return false;
    }
 ...
 */
export function deriveMasterKey(secret) {
 ...
 */
export function encryptAesGcm(plaintext, key, aad) {
 ...
 */
export function decryptAesGcm(payload, key, aad) {

...
export function loadConfig(driftDir) {
    ...
    try {
        ...
        if (core) {
            if (typeof core.version === "number")
                config.core.version = core.version;
            if (typeof core.default_model === "string")
                config.core.default_model = core.default_model;
        }
        const ast = parsed["ast"];
        if (ast) {
            if (Array.isArray(ast.parsers))
                config.ast.parsers = ast.parsers;
        ...
        if (red && Array.isArray(red.patterns)) {
            config.redaction.patterns = red.patterns;
        }
        const enc = parsed["encryption"];
        if (enc) {
            if (typeof enc.enabled === "boolean")
                config.encryption.enabled = enc.enabled;

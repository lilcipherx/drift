...
export const NO_AUDIT: ProvenanceAudit = { violations: [], replayIds: [], ambiguousIds: [] };
...
export function deriveProvenanceConclusion(input: ConclusionInput): ConclusionResult {
  ...
  const count = (state: string) => intents.filter((i) => i.signatureState === state).length;
  const parts: string[] = [
  ...
  if (failing.length > 0 || blockingKeyChange || integrityBroken) {
    ...
    for (const i of failing) {
      reasons.push(
        `${i.signatureState === "malformed" ? "malformed manifest" : `${i.signatureState} signature`} for ${(i as { id?: string }).id ?? "an intent"}`,
      );
    }

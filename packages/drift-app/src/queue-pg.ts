...
import { backoffMs, boundedError, extractTenantId, type EnqueueResult, type JobStatus, type QueueAdapter, type QueueJob, type QueueStats } from "./queue.js";
...
export class PostgresQueue implements QueueAdapter {
  ...
  async enqueue(deliveryId: string, event: string, rawBody: string, payload: unknown, signature?: string): Promise<EnqueueResult> {
    ...
    try {
      ...
      const res = await client.query(
        ...
        [deliveryId, event, rawBody, signature ?? "", JSON.stringify(payload), this.defaultMaxAttempts, now, extractTenantId(payload)],
      );
      if ((res.rowCount ?? 0) > 0) return { accepted: true, duplicate: false, alreadyProcessed: false };
      const existing = await client.query<{ status: string }>(
        "SELECT status FROM webhook_jobs WHERE delivery_id = $1",
        [deliveryId],
      );
      return {
        accepted: false,
        ...
        for (const row of claimed.rows) {
          await client.query(
            `UPDATE webhook_jobs
             ...
             WHERE id = $4`,
            [now + lease, workerId, now, Number(row.id)],
          );
        }
        await client.query("COMMIT");
        if (claimed.rows.length === 0) return [];
        const jobs = await client.query(
          `SELECT * FROM webhook_jobs WHERE id = ANY($1::bigint[]) ORDER BY id ASC`,
          [claimed.rows.map((r) => Number(r.id))],
        );
        return jobs.rows.map((r) => rowToJob(r as unknown as Record<string, unknown>));
      } catch (err) {
  ...
  async ack(id: number, result?: string): Promise<void> {
    ...
    try {
      await client.query(
        ...
        [result ? boundedError(result) : null, Date.now(), id],
      );
    } finally {
      client.release();
    }
  ...
  async nack(id: number, error: string, backoffOverrideMs?: number): Promise<"retrying" | "dead"> {
    ...
    try {
      const now = Date.now();
      const row = await client.query<{ attempts: number; max_attempts: number }>(
        "SELECT attempts, max_attempts FROM webhook_jobs WHERE id = $1",
        [id],
      );
      ...
      if (attempts >= maxAttempts) {
        await client.query(
          `UPDATE webhook_jobs
           ...
           WHERE id = $4`,
          [attempts, boundedError(error), now, id],
        );
        return "dead";
      }
      const delay =
      ...
      await client.query(
        `UPDATE webhook_jobs
         SET status = 'pending', attempts = $1, lease_until = 0, lease_owner = '',
             last_error = $2, next_attempt_at = $3, updated_at = $3
         WHERE id = $4`,
        [attempts, boundedError(error), now + Math.max(0, Math.floor(delay)), id],
      );
      return "retrying";
    } finally {
      client.release();
    ...
    try {
      await client.query(
        `UPDATE webhook_jobs
         ...
         WHERE id = $3`,
        [boundedError(error), Date.now(), id],
      );
    } finally {
      client.release();
    }
  ...
  async depth(): Promise<number> {
    ...
    try {
      const row = await client.query<{ n: string }>(
        "SELECT COUNT(*) AS n FROM webhook_jobs WHERE status IN ('pending','in_progress')",
      );
  ...
  async stats(): Promise<QueueStats> {
    ...
    try {
      const rows = await client.query<{ status: string; n: string }>(
        "SELECT status, COUNT(*) AS n FROM webhook_jobs GROUP BY status",
      );
      const counts: Record<string, number> = { pending: 0, in_progress: 0, done: 0, dead: 0 };
      for (const r of rows.rows) if (r.status in counts) counts[r.status] = Number(r.n);

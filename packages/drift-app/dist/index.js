...
import { SqliteQueue, MemoryQueue } from "./queue.js";
 ...
 */
function createQueue(env = process.env) {
    const mode = env.DRIFT_APP_QUEUE ?? "sqlite";
    if (mode === "inline")
        return null;
    if (mode === "memory")
        return new MemoryQueue({ maxAttempts: parsePositiveInt(env.DRIFT_APP_MAX_ATTEMPTS, 8) });
    if (mode === "postgres") {
        const url = env.DRIFT_QUEUE_URL ?? "";
        if (!url)
            throw new Error("DRIFT_APP_QUEUE=postgres requires DRIFT_QUEUE_URL (postgres://...)");
...
async function runStart() {
    ...
    const logger = createLogger({ level: process.env.DRIFT_APP_LOG_LEVEL ?? "info" });
